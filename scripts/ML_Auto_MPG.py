...
'''
Agenda
This tutorial is intended to walk you through all the major steps involved in
 completing an and-to-end Machine Learning project. For this project, I’ve 
 chosen a supervised learning regression problem.
...
'''
Pre-requisites and Resources
To go through this project and tutorial, you should be familiar with Machine 
Learning algorithms, Python environment setup, and common ML terminologies. 
Here are a few resources to get you started:

- Read the first 2–3 chapters of The hundred page ML book: 
    http://themlbook.com/wiki/doku.php
- List of Tasks for almost every Machine Learning Project — Keep referring to 
this list while working on this (or any other) ML project.
You need a Python Environment set up — a virtual environment dedicated to 
this project.
- You should be familiar with Jupyter Notebook.
That’s it, so make sure you have an understanding of these concepts and tools
...
Data Collection and Problem Statement
1ETE
The first step is to get your hands on the data. But if you have access to 
data (as most product-based companies do), then the first step is to define 
the problem that you want to solve. We don’t have the data yet, so we are going to collect the data first.

We are using the Auto MPG dataset from the UCI Machine Learning Repository. 
...
Once you have downloaded the data, move it to your project directory, activate 
your virtualenv, and start the Jupyter local server.

You can download the data into your project from the notebook as well 
using wget :
!wget "http://archive.ics.uci.edu/ml/machine-learning-databases/auto-mpg/auto-mpg.data"

The next step is to load this .data file into a pandas datagram. For that, 
make sure you have pandas and other general use case libraries installed. 
Import all the general use case libraries like so:
'''
...
data = df.copy()
'''
Next, look at a few rows of the dataframe and read the description of each
 attribute from the website. This helps you define the problem statement.
'''
data
'''
Problem Statement — The data contains the MPG (Mile Per Gallon) variable
 which is continuous data and tells us about the efficiency of fuel
 consumption of a vehicle in the 70s and 80s.
...
'''
Exploratory Data Analysis with Pandas and NumPy
For this rather simple dataset, the exploration is broken down into a 
series of steps:
'''
...
#%%
'''
The horsepower column has 6 missing values. We’ll have to study the 
column a bit more.
'''
...
#%%
'''
Since there are a few outliers, we can use the median of the column to impute 
the missing values using the pandas median() method.
'''
...
'''
The 2 categorical columns are Cylinders and Origin, which only have a few 
categories of values. Looking at the distribution of the values among these 
categories will tell us how the data is distributed:
'''
...
'''
The pair plot gives you a brief overview of how each variable behaves with 
respect to every other variable.

For example, the MPG column (our target variable) is negatively correlated 
with the displacement, weight, and horsepower features.
'''
#%%
### Set aside the test data set ###
'''
This is one of the first things we should do, as we want to test our final 
model on unseen/unbiased data.

There are many ways to split the data into training and testing sets but we 
want our test set to represent the overall population and not just a few 
specific categories. Thus, instead of using simple and common 
train_test_split() method from sklearn, we use stratified sampling.

Stratified Sampling — We create homogeneous subgroups called strata from 
the overall population and sample the right number of instances to each 
stratum to ensure that the test set is representative of the overall population.
In task 4, we saw how the data is distributed over each category of the 
Cylinder column. We’re using the Cylinder column to create the strata:
'''
...
'''
You can compare these results with the output of train_test_split() to find
 out which one produces better splits.
'''
#%%
'''
Checking the Origin Column
The Origin column about the origin of the vehicle has discrete values that 
look like the code of a country.

To add some complication and make it more explicit, I converted these 
numbers to strings:
'''
### Checking the Origin Column ###
...
#%%
### Testing for new variables — Analyze the correlation of each variable  ###
...
'''
We found acceleration_on_power and acceleration_on_cyl as two new variables 
which turned out to be more positively correlated than the original variables.

This brings us to the end of the Exploratory Analysis. We are ready to proceed
 to our next step of preparing the data for Machine Learning.
'''
#%%
'''
Data Preparation using Sklearn
One of the most important aspects of Data Preparation is that we have to keep 
automating our steps in the form of functions and classes. This makes it easier
 for us to integrate the methods and pipelines into the main product.
...
In order to make changes to datasets and create new variables, sklearn offers 
the BaseEstimator class. Using it, we can develop new features by defining our
 own class.
...
As I said, we want to automate as much as possible. Sklearn offers a 
great number of classes and methods to develop such automated pipelines of 
data transformations.

The major transformations are to be performed on numerical columns, so let’s 
create the numerical pipeline using the Pipeline class:
'''
...
- Standard Scaling of each Attribute — always a good practice to scale the 
values before feeding them to the ML model, using the standardScaler class.
'''
#%%
'''
Combined Pipeline for both Numerical and Categorical columns
We have the numerical transformation ready. The only categorical column we 
have is Origin for which we need to one-hot encode the values.
...
'''
To the instance, provide the numerical pipeline object created from the 
function defined above. Then call the OneHotEncoder() class to process 
the Origin column.
'''
#%%
'''
...
'''
We see that acc_on_power, which is the derived feature, has turned out to be 
...
production-ready environment, the first step is to save it into a .h5 or .bin
 file using a library like pickle .
...
#%%
'''
This will save your model in your present working directory unless you specify 
some other path.

It’s time to test if we are able to use this file to load our model and make 
predictions. We are going to use the same vehicle config as we defined above:
'''
##vehicle config
vehicle_config = {
    'Cylinders': [4, 6, 8],
...
Install the following packages:
    ...
    gunicorn
    seaborn
The next step is to activate this environment and start developing a simple 
